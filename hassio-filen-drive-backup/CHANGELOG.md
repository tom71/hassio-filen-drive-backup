...
- Added configuration options to limit the speed of uploads to Filen.io
- When Filen.io doesn't have enough space, the addon now explains how much space you're using and how much is left.  This was a source of confusion for users.
...
- Added another redundant token provider, hosted on heroku, that the addon uses for its cloud-required component when you aren't using your own Filen app credentials.
...
- Fixed misreporting of some error codes from Filen.io when a partial upload can't be resumed.
...
- Fix a potential cause of SSL errors when communicating with Filen.io
...
- Avoid global throttling of Filen.io API calls by:
  ...
  - Caching data from Filen.io for short periods during periodic syncing.
